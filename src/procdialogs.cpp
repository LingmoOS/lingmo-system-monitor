...
procdialog_create_renice_dialog (GsmApplication *app)
{
  ...
  gtk_builder_add_from_resource (builder, "/org/lingmo/lingmo-system-monitor/data/renice.ui", &err);
  if (err != NULL)
...
multi_root_check (char *command)
{
  ...
  if (procman_has_lingmosu ())
    return gsm_lingmosu_create_root_password_dialog (command);
                                        ...
                                        gint              extra_value)
{
  ...
  else if (procman_has_lingmosu ())
    ret = gsm_lingmosu_create_root_password_dialog (command);
