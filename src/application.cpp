...
GsmApplication::on_command_line (const Glib::RefPtr<Gio::ApplicationCommandLine>&command_line)
{
  ...
  if (option_group.print_version)
    {
      g_print ("%s %s\n", _("Lingmo System Monitor"), VERSION);
      exit (EXIT_SUCCESS);
    }
...
GsmApplication::on_help_activate (const Glib::VariantBase&)
{
  ...
  if (!g_app_info_launch_default_for_uri ("help:lingmo-system-monitor", NULL, &error))
    {
...
GsmApplication::on_startup ()
{
  ...
  Glib::RefPtr<Gtk::Builder> builder = Gtk::Builder::create_from_resource ("/org/lingmo/lingmo-system-monitor/data/menus.ui");
