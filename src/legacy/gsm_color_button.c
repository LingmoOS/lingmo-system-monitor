/*
 * Lingmo system monitor colour pickers
  ...
  switch (priv->type)
    {
      ...
      case GSMCP_TYPE_NETWORK_IN:
        if (priv->image_buffer == NULL)
          priv->image_buffer =
            fill_image_buffer_from_resource (cr, "/org/lingmo/lingmo-system-monitor/pixmaps/arrow_overlay.svg");
        if (priv->mask_buffer == NULL)
          priv->mask_buffer =
            fill_image_buffer_from_resource (cr, "/org/lingmo/lingmo-system-monitor/pixmaps/arrow_mask.svg");
